...
Ejecute los pasos para bifurcar a su cuenta en GitHub el repositorio que contiene este documento y posteriormente clonarlo a su computadora para editarlo con RStudio.


## Desarrollo
Realice los siguientes ejercicios de graficación en R (el valor porcentual de cada uno se muestra entre paréntesis).

1. (10%) Mediante la función ```read.csv()```, cargue en un data frame el archivo CSV con datos de casos de Covid-19
...
barplot(
  ...
  main="Cantidad de casos confirmados de Covid-19"
)
```
...
activos.ordenado<-order(ca_covid19$activos,decreasing = TRUE)
ca_covid19.activos<-ca_covid19[confirmados.ordenado,]
barplot(
...
barplot(
  ...
  xlab="País", 
  ylab="Número de habitantes", 
  main="Cantidad de casos activos de Covid-19"
)
...
barplot(
  ...
  xlab="País", 
  ylab="Número de habitantes", 
  main="Cantidad de casos fallecidos de Covid-19"
)
